...
unsigned   spell_out(word, n, phone) char *word;
int        n;
darray_ptr phone;
{
	unsigned nph = 0;
	fprintf(stderr, "Spelling '%.*s'\n", n, word);
	while (n-- > 0) { nph += xlate_string(ASCII[*word++ & 0x7F], phone); }
	return nph;
}

int suspect_word(s, n) char *s;
int n;
{
	int i          = 0;
	int seen_lower = 0;
	...
	for (i = 0; i < n; i++) {
		char ch = *s++;
		if (i && last != '-' && isupper(ch)) seen_upper = 1;
		if (islower(ch)) {
			seen_lower = 1;
			ch         = toupper(ch);
		}
		if (ch == 'A' || ch == 'E' || ch == 'I' || ch == 'O' || ch == 'U' || ch == 'Y') seen_vowel = 1;
		last = ch;
	}
...
static unsigned xlate_word(word, n, phone) char *word;
int             n;
darray_ptr      phone;
{
	unsigned nph = 0;
	if (*word != '[') {
		...
		if (suspect_word(word, n))
			...
			if (dict || verbose) fprintf(stderr, "Guess %p '%.*s'\n", dict, n, word);
...
void say_phones(phone, len, verbose) char *phone;
int  len;
int  verbose;
{
	darray_t elm;
	...
	if ((frames = phone_to_elm(phone, len, &elm))) {
		...
		if (samp) {
			unsigned nsamp = holmes(elm.items, (unsigned char *)darray_find(&elm, 0), max_samples, samp);
			audio_play(nsamp, samp);
			if (file_write) (*file_write)(nsamp, samp);
			free(samp);
		}
	}
	darray_free(&elm);
...
unsigned   xlate_string(string, phone) char *string;
darray_ptr phone;
{
	unsigned nph = 0;
	char *   s   = string;
	char     ch;
	while (isspace(ch = *s)) s++;
	while ((ch = *s)) {
		char *word = s;
		if (isalpha(ch)) {
			while (isalpha(ch = *s) || ((ch == '\'' || ch == '-' || ch == '.') && isalpha(s[1]))) s++;
			if (!ch || isspace(ch) || ispunct(ch) || (isdigit(ch) && !suspect_word(word, s - word)))
				nph += xlate_word(word, s - word, phone);
			else {
				while ((ch = *s) && !isspace(ch) && !ispunct(ch)) s++;
			...
			case '[': {
				...
				if (e) {
					s++;
					while (s < e) phone_append(phone, *s++);
					s = e + 1;
					break;
				}
			}
...
char *concat_args(argc, argv) int argc;
char *argv[];
{
	int   len = 0;
	int   i;
...
void say_string(s) char *s;
{
	darray_t phone;
	darray_init(&phone, sizeof(char), 128);
	xlate_string(s, &phone);
	if (phone.items) say_phones((char *)darray_find(&phone, 0), phone.items, verbose);
	darray_free(&phone);
}

extern int darray_fget PROTO((FILE * f, darray_ptr p));

int        darray_fget(f, p) FILE *f;
darray_ptr p;
{
	int ch;
